...
# Poster Press palette, matching poster-press.css so plots sit flush on the
# page background rather than in a lighter rectangle.
pp_ink       <- "#1b1b14"
pp_ink_card  <- "#211f16"
pp_rule      <- "#2c2c20"
pp_cream     <- "#ece3cc"
...
theme_dark_site <- function(...) {
  ...
  ggplot2::theme(
    plot.background   = ggplot2::element_rect(fill = pp_ink, colour = NA),
    panel.background  = ggplot2::element_rect(fill = pp_ink, colour = NA),
    panel.grid.major  = ggplot2::element_line(colour = pp_rule),
    panel.grid.minor  = ggplot2::element_line(colour = pp_ink_card),
    text              = ggplot2::element_text(colour = pp_body),
    axis.text         = ggplot2::element_text(colour = pp_muted),
    plot.title        = ggplot2::element_text(colour = pp_cream, face = "bold"),
    plot.subtitle     = ggplot2::element_text(colour = pp_muted),
    plot.caption      = ggplot2::element_text(colour = pp_muted_2),
    legend.background = ggplot2::element_rect(fill = pp_ink, colour = NA),
    legend.key        = ggplot2::element_rect(fill = pp_ink, colour = NA),
    legend.text       = ggplot2::element_text(colour = pp_body),
    legend.title      = ggplot2::element_text(colour = pp_body),
    strip.background  = ggplot2::element_rect(fill = pp_ink_card, colour = NA),
    strip.text        = ggplot2::element_text(colour = pp_body)
  )
}
```
...
|------------------------------------------|-----------------------------------------------------|----------------------------------------------|
| *Coherence up but Exclusivity down*    | Topics are interpretable but overlapping            | Try slightly lower `K`                        |
| *Exclusivity up but Coherence down*    | Topics are distinct but fragmented or incoherent    | Try slightly lower or slightly higher `K`     |
...
ggplot(topic_df, aes(x = Label, y = Mean)) +
  geom_col(fill = pp_amber) +
  geom_errorbar(aes(ymin = Mean - SD, ymax = Mean + SD), width = 0.2, color = pp_cream) +
  labs(x = "Topic",
       y = "Expected %\nof Corpus") +
  scale_y_continuous(labels = scales::percent) +
  theme_dark_site(base_size = 12) +
...
ggplot(df, aes(label = word, size = prob)) +
  geom_text_wordcloud(area_corr = TRUE, color = pp_amber) +
